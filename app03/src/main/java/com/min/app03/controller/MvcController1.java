...
public class MvcController1 {
   ...
   *   3) ModelAndView : 사용자에게 보여줄 JSP의 경로와 이름은 setViewName(); 메소드로 저장하고, 전달할 값은 addObject() 로 저장한다.
   *   4) ...
   * 
   * 메소드명
   *   아무런 의미가 없다.
  ...
  public void letsgo() {
    // 반환 타입이 void 이므로 요청주소 "/webdir1/list" 를 사용자에게 보여줄 JSP 경로와 이름으로 해석.
    // ViewResolver 에 의해서 최종 경로는 "/WEB-INF/views/webdir1/list.jsp" 가 됩니다.
  }
  
 @RequestMapping(value = "/board/view")  // method를 생략하면 디폴트인 GET방식으로 동작함.,
  public ModelAndView gogogo() {
    
    ModelAndView mav = new ModelAndView();
    mav.setViewName("webdir1/detail");    //  사용자에게 보여줄 JSP의 경로와 이름. ViewResolver에 의해 아ㅠ뒤
    mav.addObject("number", 10);          //  JSP로 number = 10 이라는 값을 전달.
    return mav;
