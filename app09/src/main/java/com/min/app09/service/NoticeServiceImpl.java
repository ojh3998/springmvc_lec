...
@Service  // controller 가 서비스를 사용할껀데 그러기위해서는 service도 bean으로 만들어주어야함.
public class NoticeServiceImpl implements INoticeService {
  ...
  public List<NoticeDto> getNoticeList() {
    ...
    return noticeDao.selectNoticeList(); // DB로부터 그대로받아와서 전달
  }
  ...
  public String registNotice(MultipartHttpServletRequest multipartRequest) {
    ...
    // DB에 공지사항 등록하기
    int result = noticeDao.insertNotice(noticeDto); // 매퍼의<selectKey>에 의해서 noticeDto의 noticeId 필드에 값이 채워집니다.
    if(result == 0)
      return "공지사항 등록 실패";
    ...
    for(MultipartFile multipartFile : files) {
      ...
      // 첨부 파일 존재 여부 확인
      if(!multipartFile.isEmpty()) { // 비어있지 않으면 즉, 존재하면
  ...
  public ResponseEntity<Resource> download(int attachId, String userAgent) {
    ...
    try {
      responseHeader.add("Content-Disposition", "attachment; filename=" + originalFilename); // 다운로드 응답을 의미합니다. 다운로드 할 파일명을  originalFilename로 결정합니다.
      responseHeader.add("Content-Type", "application/octet-stream");                        // 응답 데이터의 타입을 의미합니다. 일반 바이너라 파일을 의미합니다.
