...
@Component // spring container에 bean으로 만들기
public class FileUtil {
	...
	/** 파일 업로드 경로를 반환하는 메소드
	 *  @return 현재 날짜를 경로로 사용. 예를들어 2024-12-12일에 실행하는 경우 반환되는 경로는 /upload/2024/12/12 입니다.
	 ...
	 * 파일 저장 이름을 반환하는 메소드
	 * @param 파일의 원래 이름
	 * @return 파일의 저장이름. 중복 방지를 위해서 난수 처리된 이름을 사용(UUID). 파일의 원래 확장자는 그대로 사용
	 */
