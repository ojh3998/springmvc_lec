...
public class PageUtil {
  ...
  private int total;    // 전체 목록의 갯수 (SELECT문 실행)
  private int totalPage;// 전체 페이지 갯수 (total과 display에 의해서 계산되는 항목) 
  private int beginPage;// 각 블록의 시작페이지 (page와 pagePerBlock에 의해서 계산되는 항목)
  private int endPage;  // 각 블록의 종료페이지 (beginPage와 pagePerBlock과 totalPage에 의해서 계산되는 항목)
  
  // service에서 호출할 method
  /**
   * 서비스로부터 page와 display와  total 값을 받아온 뒤 
   * 페이징 처리에 필요한 모든 값을 구하는 메소드 
  ...
  public void setPaging(int page, int display, int total) {
    ...
    int pagePerBlock = 10;
    totalPage = (int)Math.ceil((double)total / display);    // Math.ceil 올림처리
  ...
  /**
   * 호출한 서비스 페이지이동 링크를 문자열 형식으로 반환하는 메소드
  ...
  public String getPaging(String requestURI, String sort) {
    ...
    builder.append(".paging button { display: block; border: none; background-color: #fff; text-align: center; width: 30px; height: 30px; line-hight: 30px; cursor: pointer; }");
    ...
    // <button type="button" onclick="location.href='/app10/user/list.do?page=4'">4</button> 
    // <button type="button" onclick="location.href='/app10/user/list.do?page=5'">5</button> 
    // <button type="button" onclick="location.href='/app10/user/list.do?page=6'">6</button> 
    // <button type="button" onclick="location.href='/app10/user/list.do?page=7'">7</button> 
    // <button type="button" onclick="location.href='/app10/user/list.do?page=8'">8</button> 
    // <button type="button" onclick="location.href='/app10/user/list.do?page=9'">9</button> 
    // <button type="button" onclick="location.href='/app10/user/list.do?page=10'">10</button> 
