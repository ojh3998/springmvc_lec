...
@RequiredArgsConstructor // final 필드로 선언된 IBoardDao boardDao에 자동으로 bean을 주입하기 위한 생성자 입니다.
@Service // service에서 사용하는 @Component입니다. Spring Container에 IBoardServie 타입의 bean이 생성 됩니다.
public class BoardServiceImpl implements IBoardService {
	
	private final IBoardDao boardDao;

	@Override
	public Map<String, Object> getBoardList(HttpServletRequest repuest) {
		
		// 요청 파라미터 처리 (sort가 전달되지 않으면 DESC를 사용합니다.)
		Optional<String> opt = Optional.ofNullable(repuest.getParameter("sort"));  //DAO로 보낼 sort 를 생산
	...
	public String removeBoard(int boardId) {
		...
		// 삭제한 뒤 결과를 텍스트로 반환합니다.
		return boardDao.deleteBoard(boardId) == 1 ? "삭제 성공" : "삭제 실패";
		
	}
	
	@Override
	public String remobeBoardList(String[] numbers) {
	...
	public void txTest() {
	   ...
	   * 트랜잭션 처리가 필요한 경우
	   * 
	   * INSERT/UPDATE/DELETE 처리가 2개이상으로 이루어지는 서비스입니다.
